...
#define DIABATIC(i,j,k) ( m_diabatic[INDEX(i,j,k)] )
...
double *pp_hor_adv,*pp_vert_adv,*pb_hor_adv,*pb_vert_adv,*bp_hor_adv,*bp_vert_adv,*tdiff,*t_diabatic;
...
double *pe_pp_hor_adv,*pe_pp_vert_adv,*pe_pb_hor_adv,*pe_pb_vert_adv,*pe_bp_hor_adv,*pe_bp_vert_adv,*pe_tdiff,*pe_diabatic;
double *pe_cond;
...
// Vorticity budget
//---------------------------------------------------
char vorticity_budget_filename[len];
...
void initialize_budgets(){
	...
	if(grid_ratio > 1 && !smallArrayIsInitialized){
	...
	if(HEAT_BUDGET || PE_BUDGET || PV_BUDGET){ m_diabatic = ALLOC(fNX*fNY*fNZ);}

	bool budgetsRequested = false;

	if(HEAT_BUDGET || PE_BUDGET || PV_BUDGET || VORTICITY_BUDGET || MOISTURE_BUDGET || PV_TRACER){
		budgetsRequested = true;	
	}

	if(budgetsRequested && rank==0){ printf("-----------------------------------------------------\n");}
	...
	**********************************************/
	if(HEAT_BUDGET){
		...
		if(inputs.has_shear){ add_commandline_arguments(heat_budget_filename,inputs.heat_budget_filename);} 
		else {			 strcpy(heat_budget_filename,inputs.heat_budget_filename);}
	
		if(rank==0){
		
			printf("Heat budget file name = %s\n",heat_budget_filename);
			
			if(!isRestartRun){ create_budget_file(heat_budget_filename,0);}
		}
	...
	**********************************************/
	if(PE_BUDGET){
		...
		if(inputs.has_shear){ add_commandline_arguments(pe_budget_filename,inputs.pe_budget_filename);} 
		else {			 strcpy(pe_budget_filename,inputs.pe_budget_filename);}
	
		if(rank==0){
		
			printf("Potential energy budget file name = %s\n",pe_budget_filename);
			
			if(!isRestartRun){ create_budget_file(pe_budget_filename,6);}
		}
	...
	**********************************************/
	if(MOISTURE_BUDGET){
		...
		if(inputs.has_shear){ add_commandline_arguments(moisture_budget_filename,inputs.moisture_budget_filename);} 
		else {			 strcpy(moisture_budget_filename,inputs.moisture_budget_filename);}
	
		if(rank==0){
						
			printf("Moisture budget file name = %s\n",moisture_budget_filename);
			
			if(!isRestartRun){ create_budget_file(moisture_budget_filename,5);}
		}
	...
	**********************************************/
	if(VORTICITY_BUDGET){
		...
		strcpy(vorticity_budget_filename,inputs.vorticity_budget_filename);
	
		if(rank==0){
			if(!isRestartRun){ create_budget_file(vorticity_budget_filename,1);}
		}
	...
	**********************************************/
	if(PV_BUDGET){ 
		...
		if(inputs.has_shear){ add_commandline_arguments(pv_budget_filename,inputs.pv_budget_filename);} 
		else {			 strcpy(pv_budget_filename,inputs.pv_budget_filename);}
	
		if(rank==0){ 
			printf("PV budget file name = %s\n",pv_budget_filename);
			
			if(!isRestartRun){ create_budget_file(pv_budget_filename,2);}
		}
	...
	**********************************************/
	if(PV_TRACER){ 
		...
		if(rank==0){ 
			if(!isRestartRun){ create_budget_file(pv_tracer_filename,4);}
		}
...
void calculate_budgets(int substep,double *steps){
	...
	if(substep==2 && (HEAT_BUDGET || PE_BUDGET) ){ calculate_heat_budget(bound,fNX-bound,bound,fNY-bound);}
	if(substep==2 && MOISTURE_BUDGET){ calculate_moisture_budget(bound,fNX-bound,bound,fNY-bound);}
	if(substep==2 && VORTICITY_BUDGET){ calculate_vorticity_budget(bound,fNX-bound,bound,fNY-bound);}
	if(substep==2 && PV_BUDGET){ calculate_pv_budget(bound,fNX-bound,bound,fNY-bound);}
...
void calculate_heat_budget(int il,int ih,int jl,int jh){
	...
	for(int k=1;k<NZ-2;k++){
		...
		if(PE_BUDGET){	
			...
			pe_diabatic[ind] 	+= m_diabatic[ind] * pe_mult;
		}
	}}}
...
void calculate_pv_budget(int il,int ih,int jl,int jh){
	...
	for(int k=1;k<NZ-1;k++){
		...
		//----------------------------------------------------------
		scalar_gradient_vector(m_diabatic,i,j,k,&dthetaDot);

		//----------------------------------------------------------
		// Potential temperature gradient vector
		//----------------------------------------------------------
		scalar_gradient_vector(ths,i,j,k,&dtheta);

		scalar_gradient_vector(m_thbar,i,j,k,&dTheta);

		scalar_gradient_vector(t_diffusion,i,j,k,&dthetaF);
		...
		//----------------------------------------------------------
		pv_diff[ind] += ( DOT_PRODUCT(zetaF,dTheta) + DOT_PRODUCT(zetaF,dtheta) + DOT_PRODUCT(Zeta,dthetaF) + DOT_PRODUCT(zeta,dthetaF) ) * one_d_rhou[k] * dt;
...
void write_tvar_to_file_laplacian(const char *myfilename,const char *varname,double *var,bool zero_out){
	...
	if(rank==0){
		...
		double *input = (double*) calloc(NX*NY,sizeof(double));
...
void write_PVvars_to_file(const char *myfilename){
	...
	if(rank==0){ write_time_to_file(myfilename,file_time_counter);}
...
void write_pevars_to_file(const char *myfilename){
	...
	write_tvar_to_file(myfilename,"pe_m_diabatic",pe_diabatic,true);
...
void write_tvars_to_file(const char *myfilename){
	...
	write_tvar_to_file(myfilename,"m_diabatic",t_diabatic,true);
...
void write_qvars_to_file(const char *myfilename){
	...
	write_tvar_to_file(myfilename,"qm_diabatic",q_diabatic,true);
...
void get_nondivergent_winds(double *us,double *vs,double *vort){
	...
	for(int k=0;k<NZ;k++){
		...
		for(int j=0;j<NY;j++){
			...
			input[i*NY+j] = vort[INDEXB(i,j,k)];
		...
		for(int j=1;j<NY-1;j++){
			...
			vort[INDEXB(i,j,k)] = input[i*NY+j];

			U(i,j,k) = -0.5*one_d_dy * (input[i*NY+j+1] - input[i*NY+j-1]);
			V(i,j,k) = 0.5*one_d_dx * (input[(i+1)*NY+j] - input[(i-1)*NY+j]);
...
CubicInterpolate2(avortVector[INDEX(i-1,j,k)].i,avortVector[INDEX(i,j,k)].i,avortVector[INDEX(i+1,j,k)].i,avortVector[INDEX(i+2,j,k)].i,0.5)								   
							...
							-  
																//(avortVector[INDEX(i,j,k)].i+avortVector[INDEX(i-1,j,k)].i)	*
CubicInterpolate2(avortVector[INDEX(i-2,j,k)].i,avortVector[INDEX(i-1,j,k)].i,avortVector[INDEX(i,j,k)].i,avortVector[INDEX(i+1,j,k)].i,0.5)*
																CubicInterpolate2(DIABATIC(i-2,j,k),DIABATIC(i-1,j,k),DIABATIC(i,j,k),DIABATIC(i+1,j,k),0.5)
						) * one_d_dx 
								   +
...
/*********************************************************************
* Create the output file for budgets.
*
* @param type
*				0 - heat budget
*				1 - vorticity budget
*				2 - pv budget
*				3 - Omega equation
*				4 - PV tracer equation
*				5 - Moisture budget
*				6 - Potential energy budget
*
**********************************************************************/
void create_budget_file(const char *myfilename,int type){
	...
	**************************************************/
	...
	status = nc_def_var (ncid,"zu", NC_FLOAT, 1, var_dimids, &var_id);
	if (status != NC_NOERR) handle_error(status);
	...
	*********************************************************/
	...
	status = nc_inq_varid (ncid,"zu", &var_id);
	if (status != NC_NOERR) handle_error(status);

	status = nc_put_var_double(ncid, var_id, &ZU(0));
	if (status != NC_NOERR) handle_error(status);
