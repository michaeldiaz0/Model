...
void run_laplacian_solver3d(int ni,int nj,int nk,double *input){
	...
	*****************************************************/
	...
	for(int k=0;k<nk;k++){
		...
		IN3D(i,j,k)[0] = input[INDEX(i,j,k)];
		IN3D(i,j,k)[1] = 0;
	}}}
	...
	***********************************************/
	...
	for(int j=0;j<nj;j++){
		...
		for(int k=2;k<nk-1;k++){
			...
			OUT3D(i,j,k)[0] = (OUT3D(i,j,k)[0] - acoef3d[k]*OUT3D(i,j,k-1)[0])*z;
			OUT3D(i,j,k)[1] = (OUT3D(i,j,k)[1] - acoef3d[k]*OUT3D(i,j,k-1)[1])*z;
		}
	}}
	...
	*****************************************************/
	...
	for(int k=0;k<nk;k++){
		...
		input[INDEX(i,j,k)] = IN3D(i,j,k)[0];
