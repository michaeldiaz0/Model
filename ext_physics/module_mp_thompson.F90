...
!.. Thompson tables.
      character(len=*), parameter :: thomp_table_file = 'ext_physics/thompson_tables_precomp_v2.sl'
      character(len=*), parameter :: qr_acr_qg_file = 'ext_physics/qr_acr_qgV2.dat'
      character(len=*), parameter :: qr_acr_qs_file = 'ext_physics/qr_acr_qsV2.dat'
      character(len=*), parameter :: freeze_h2o_file = 'ext_physics/freezeH2O.dat'
...
!> @{
      SUBROUTINE thompson_init(is_aerosol_aware_in,       &
                               ...
                               threads, errmsg, errflg) bind(c,name="thompson_init")
      ...
      INTEGER(c_int), INTENT(IN) :: mpicomm, mpirank, mpiroot
      INTEGER(c_int), INTENT(IN) :: threads
      CHARACTER(len=1), INTENT(INOUT) :: errmsg
...
!> @{
      subroutine mp_thompson (qv1d, qc1d, qi1d, qr1d, qs1d, qg1d, ni1d,    &
                          ...
                          tprr_rcs1, tprv_rev1,                            &
                          tten1, qvten1, qrten1, qsten1,                   &
                          qgten1, qiten1, niten1, nrten1, ncten1, qcten1) bind(c,name="mp_thompson")
...
!..Sub arguments
      ...
      REAL(c_double), DIMENSION(kts:kte), INTENT(INOUT):: &
                          qv1d, qc1d, qi1d, qr1d, qs1d, qg1d, ni1d, &
                          nr1d, nc1d, nwfa1d, nifa1d, t1d
      REAL(c_double), DIMENSION(kts:kte), INTENT(IN):: p1d, w1d, dzq
      REAL(c_double), INTENT(INOUT):: pptrain, pptsnow, pptgraul, pptice
      REAL(c_double), INTENT(IN):: dt
      REAL(c_double), INTENT(IN):: rand1, rand2, rand3
      ! Extended diagnostics, most arrays only allocated if ext_diag is true
      LOGICAL(c_bool), INTENT(IN) :: ext_diag
      LOGICAL(c_bool), INTENT(IN) :: sedi_semi
      INTEGER(c_int), INTENT(IN) :: decfl
      REAL(c_float), DIMENSION(:), INTENT(OUT):: &
                          !vtsk1, txri1, txrc1,                       &
                          prw_vcdc1,                                 &
                          prw_vcde1, tpri_inu1, tpri_ide1_d,         &
                          tpri_ide1_s, tprs_ide1,                    &
...
!+---+-----------------------------------------------------------------+ !  DROPLET NUCLEATION
           if (clap .gt. eps) then
            if (is_aerosol_aware) then
               xnc = DMAX1(2., activ_ncloud(temp(k), w1d(k)+rand3, nwfa(k)))
            else
               xnc = Nt_c
            endif
...
!..Local variables
 ...
 2010   CONTINUE
      ...
      ENDIF
        ...
        OPEN(iunit_mp_th1,FILE='ext_physics/CCN_ACTIVATE.BIN',                      &
...
!.. and its tendency to diagnose a pretend postive updraft velocity.
      double precision function activ_ncloud(Tt, Ww, NCCN)

      implicit none
      DOUBLE PRECISION, INTENT(IN):: Ww
      real, intent(in) :: Tt, NCCN
...
!
      ...
      double precision, intent(in) :: dzl(km)
      real, intent(in) :: wwl(km)
