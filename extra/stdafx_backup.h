#pragma once



// initialization from model output will give slightly different
// results compared to running straight through because the output
// time is at the end of the time step.

// periodic boundaries for parallel and serial not compatible, don't 
// initialize serial version with output from parallel version or vice versa.

// still need to make latoffset and lonoffset consistent amoung initialization options

#if 1
const int mult = 5;
const int NX = 750;//300;//600;//75*mult;// + 22;//200;//800;
const int NY = 294;//148;//225;//37*mult+0;//70;//240;
const int NZ = 43;//40;//43;

const double dx = 12000;//75000/(double)mult;
const double dy = dx;
const double dz = 500;
const double dt = 40;//112;//112;//20;//450/(double)mult;

const double lonoffset = 60;
const double latoffset = 92;//95;
const int number_of_time_steps = 8*24*60*60/(int)dt;

const int raydampheight = NZ - 5;
const int outfilefreq = 3*60*60/(int)dt;
const int stopheating = -576;
#endif

#if 0
const int mult = 1;

const int NX = 555;
const int NY = 280;
const int NZ = 45;

const double dx = 15000/(double)mult;
const double dy = dx;
const double dz = 500;
const double dt = 90/(double)mult;

const double lonoffset = 55;
const double latoffset = 90;
const int number_of_time_steps = 20*24*60*60/(int)dt + 3;

const int raydampheight = NZ - 5;
const int outfilefreq = 6*60*60/(int)dt;
const int stopheating = -576;
#endif

#if 0
const int NX = 400;//200;//800;
const int NY = 120-2;//70;//240;
const int NZ = 40;

const double dx = 50000;
const double dy = dx;
const double dz = 500;
const double dt = 300;

const double lonoffset = 110;
const double latoffset = 90+20;
const int number_of_time_steps = 6*24*60*60/(int)dt;

const int raydampheight = NZ - 5;
const int outfilefreq = 1.0*60*60/(int)dt;
const int stopheating = -576;
#endif

#if 0
const int NX = 250;
const int NY = 183;// + 88/3;
const int NZ = 43;

const double dx = 15000;
const double dy = 15000;
const double dz = 500;
const double dt = 40;

const double lonoffset = 70;
const double latoffset = 96;
const int number_of_time_steps = 6*24*60*60/(int)dt;

const int raydampheight = NZ - 5;
const int outfilefreq = 1.0*60*60/(int)dt;
const int stopheating = -576*2;
#endif

#if 0
const int NX = 100*2;
const int NY = 40*2;// + 88/3;
const int NZ = 100;

const double dx = 100000/2.0;
const double dy = 100000/2.0;
const double dz = 250;
const double dt = 600;

const double lonoffset = 45;
const double latoffset = 93;
const int number_of_time_steps = 27000000;

const int raydampheight = NZ - 5;
const int outfilefreq = 36*4;
const int stopheating = -576;
#endif

//--------------------------------------------------------
// Grid stretching
//--------------------------------------------------------
const double height_lowest_level = 100;//200; // height of lowest full level in meters
const int index_lowest_level = 1;		// what is the index of this level?
#if 0
//--------------------------------------
// Version to execute
//--------------------------------------
#define PARALLEL 0
#define ENSEMBLE 0
#define ENERGY 0
//--------------------------------------
// Equation set
//--------------------------------------
#define HYDROSTATIC 0			// hydrostatic option (no longer works)
#define ISLINEAR 1				// linearize equation set
#define USE_LINEAR_FRICTION 1	// linear friction in lowest model level(s)
#define USE_TURBULENT_STRESS 0	// use turbulence parameterization
#define RESTING_BASIC_STATE 0	// set all basic state terms to zero
//--------------------------------------
// Physics options
//--------------------------------------
#define MICROPHYSICS_OPTION 1		// 0:None 1:Kessler 2:Rutludge
#define USE_TERRAIN 0				// 0:no, 1:yes
#define SURFACE_HEAT_FLUX 0			// 0:no, 1:yes
#define USE_LANDSEA_FROM_FILE 0		// 0:no, 1:yes
//--------------------------------------
// For linearized equation set
//--------------------------------------
#define OUTPUT_DIFFUSION_TEND 0		// 
#define EXTRA_DIFFUSION 1			// only for linearized version, no grid stretching
#define FOURIER_DAMPING 1			// use Fourier transform to specify wavenumber
#define WAVE_NUMBER 3				// the specified wavenumber
//--------------------------------------
// Grid / boundaries
//--------------------------------------
#define PERIODIC_BOUNDARIES 1
#define MERIDIONAL_CROSS_SECTION 0
#define STRETCHED_GRID 0
#define SHIFT_PRIME_MERIDIAN 0
//--------------------------------------
// Initialization Options
//--------------------------------------
#define BASIC_STATE_OPTION 1			// 0:Model, 1:Code, 2:Reanalysis 
#define PERTURBATION_OPTION 1			// 0:Model, 1:Code
#define REANALYSIS_INITIALIZE_OPTION 1 	// 1:processed ERA-interim 
										// 2:NCAR-reanalysis monthly mean 
										// 3:ERA5 monthly mean 
										// 4:ERA5 processed
//--------------------------------------
// Output
//--------------------------------------
#define VERBOSE 1					// lots of output to screen?
#define PV_BUDGET 0					// calculate potential vorticity budget and write to a file
#define HEAT_BUDGET 0				// calculate heat budget and write to a file
#define MOISTURE_BUDGET 0			// calculate heat budget and write to a file
#define VORTICITY_BUDGET 0			// calculate vorticity budget and write to a file
#define PV_TRACER 0
#define OUTPUT_FRICTION_TEND true	// output frictional tendencies
#define OUTPUT_TO_FILE true			// write output to netcdf file
#define PRINT_EKE_BUDGET 0			// print EKE budget to terminal window
#define CALCULATE_OMEGA 0
#define EXTRA_OUTPUT 0
#define IS_MAC 1

#define FFTW_FLAGS FFTW_PATIENT
#endif

#if 1
//--------------------------------------
// Version to execute
//--------------------------------------
#define PARALLEL 0
#define ENSEMBLE 0
#define ENERGY 0
//--------------------------------------
// Equation set
//--------------------------------------
#define HYDROSTATIC 0			// hydrostatic option (no longer works)
#define ISLINEAR 0				// linearize equation set
#define USE_LINEAR_FRICTION 0	// linear friction in lowest model level(s)
#define USE_TURBULENT_STRESS 1	// use turbulence parameterization
#define RESTING_BASIC_STATE 0	// set all basic state terms to zero (doesn't work with some options)
//--------------------------------------
// Physics options
//--------------------------------------
#define MICROPHYSICS_OPTION 2		// 0:None 1:Kessler 2:Rutludge
#define USE_TERRAIN 0				// 0:no, 1:yes
#define SURFACE_HEAT_FLUX 0			// 0:no, 1:yes
#define USE_LANDSEA_FROM_FILE 0		// 0:no, 1:yes
//--------------------------------------
// For linearized equation set
//--------------------------------------
#define OUTPUT_DIFFUSION_TEND 0		// 
#define EXTRA_DIFFUSION 0			// only for linearized version, no grid stretching
#define FOURIER_DAMPING 0			// use Fourier transform to specify wavenumber
#define WAVE_NUMBER 3				// the specified wavenumber
//--------------------------------------
// Grid / boundaries
//--------------------------------------
#define PERIODIC_BOUNDARIES 1
#define MERIDIONAL_CROSS_SECTION 0
#define STRETCHED_GRID 1
#define SHIFT_PRIME_MERIDIAN 0
//--------------------------------------
// Initialization Options
//--------------------------------------
#define BASIC_STATE_OPTION 1			// 0:Model output, 1:Code, 2:Reanalysis 
#define PERTURBATION_OPTION 1			// 0:Model output, 1:Code
#define REANALYSIS_INITIALIZE_OPTION 1 	// 1:processed ERA-interim 
										// 2:NCAR-reanalysis monthly mean 
										// 3:ERA5 monthly mean 
										// 4:ERA5 processed
//--------------------------------------
// Output
//--------------------------------------
#define VERBOSE 1					// lots of output to screen?
#define PV_BUDGET 0					// calculate potential vorticity budget and write to a file
#define HEAT_BUDGET 0				// calculate heat budget and write to a file
#define MOISTURE_BUDGET 0			// calculate moisture budget and write to a file
#define VORTICITY_BUDGET 0			// calculate vorticity budget and write to a file
#define PV_TRACER 0
#define OUTPUT_FRICTION_TEND true	// output frictional tendencies
#define OUTPUT_TO_FILE true			// write output to netcdf file
#define CREATE_NEW_OUTPUT_FILE true
#define PRINT_EKE_BUDGET 0			// print EKE budget to terminal window
#define CALCULATE_OMEGA 0
#define EXTRA_OUTPUT 0
#define FILEBASE 1		// 0 - mac, 1 - savio, 2 - NERSC

#define FFTW_FLAGS FFTW_PATIENT

#endif


#if 0
//--------------------------------------
// Version to execute
//--------------------------------------
#define PARALLEL 0
#define ENSEMBLE 0
#define ENERGY 0
//--------------------------------------
// Equation set
//--------------------------------------
#define HYDROSTATIC 0			// hydrostatic option (no longer works)
#define ISLINEAR 1				// linearize equation set
#define USE_LINEAR_FRICTION 1	// linear friction in lowest model level(s)
#define USE_TURBULENT_STRESS 0	// use turbulence parameterization
#define RESTING_BASIC_STATE 0	// set all basic state terms to zero
//--------------------------------------
// Physics options
//--------------------------------------
#define MICROPHYSICS_OPTION 0		// 0:None 1:Kessler 2:Rutludge
#define USE_TERRAIN 1				// 0:no, 1:yes
#define SURFACE_HEAT_FLUX 0			// 0:no, 1:yes
#define USE_LANDSEA_FROM_FILE 0		// 0:no, 1:yes
//--------------------------------------
// For linearized equation set
//--------------------------------------
#define OUTPUT_DIFFUSION_TEND 0		// 
#define EXTRA_DIFFUSION 1			// only for linearized version, no grid stretching
#define FOURIER_DAMPING 0			// use Fourier transform to specify wavenumber
//--------------------------------------
// Grid / boundaries
//--------------------------------------
#define PERIODIC_BOUNDARIES 0
#define MERIDIONAL_CROSS_SECTION 0
#define STRETCHED_GRID 0
#define SHIFT_PRIME_MERIDIAN 0
//--------------------------------------
// Initialization Options
//--------------------------------------
#define BASIC_STATE_OPTION 0			// 0:Model, 1:Code, 2:Reanalysis 
#define PERTURBATION_OPTION 1			// 0:Model, 1:Code
#define REANALYSIS_INITIALIZE_OPTION 1 	// 1:processed ERA-interim 
										// 2:NCAR-reanalysis monthly mean 
										// 3:ERA5 monthly mean 
										// 4:ERA5 processed
//--------------------------------------
// Output
//--------------------------------------
#define VERBOSE 1					// lots of output to screen?
#define PV_BUDGET 0					// calculate potential vorticity budget and write to a file
#define HEAT_BUDGET 0				// calculate heat budget and write to a file
#define VORTICITY_BUDGET 0			// calculate vorticity budget and write to a file
#define PV_TRACER 0
#define OUTPUT_FRICTION_TEND true	// output frictional tendencies
#define OUTPUT_TO_FILE true			// write output to netcdf file
#define PRINT_EKE_BUDGET 0			// print EKE budget to terminal window
#define CALCULATE_OMEGA 0
#define EXTRA_OUTPUT 0
#define IS_MAC 1

#define FFTW_FLAGS FFTW_PATIENT
#endif

//--------------------------------------
// Where is the input and output?
//--------------------------------------
#if FILEBASE == 0
	#define INPUT_FILE_PATH "/Users/michaeldiaz/Desktop/model_input/"
	#define OUTPUT_FILE_PATH "/Users/michaeldiaz/Desktop/model_output/"
#elif FILEBASE == 1
	#define INPUT_FILE_PATH "../model_input/"
	#define OUTPUT_FILE_PATH "/global/scratch/michaeldiaz/"
#elif FILEBASE == 2
	#define INPUT_FILE_PATH "/global/cscratch1/sd/michaeld/"
	#define OUTPUT_FILE_PATH "/global/cscratch1/sd/michaeld/"
#endif

// C RunTime Header Files
#include <stdlib.h>
#include <memory.h>
#include <math.h>
#include <stdio.h>
#include <ctime>
#include <string.h>

// Model Header Files
#include "model.h"
#include "pvars.h"
#include "microphysics.h"
#include "file.h"
#include "util.h"
#include "solver.h"
