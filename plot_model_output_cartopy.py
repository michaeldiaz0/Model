...
def plot_winds_temperature_pressure(infile,outfile,time):
    ...
    fig,ax = plt.subplots(ncols=1,nrows=1,figsize=(18,14),subplot_kw={'projection': ccrs.PlateCarree(central_longitude=0)})

    ax.gridlines(draw_labels=True, dms=False,x_inline=False, y_inline=False,linewidth=0.5, color='gray', alpha=0.5, linestyle='--')

    #-----------------------------------
    # make filled contour plot
    #-----------------------------------
    cs = ax.contourf(lons,lats,temp,cmap=plt.cm.bwr, extend='both')
    cb = plt.colorbar(cs, shrink=0.5)
    
    #-----------------------------------
    # make line contour plot
    #-----------------------------------    
    cs = ax.contour(lons,lats,p,linewidths=1,colors='k')
    ...
    quiv = ax.quiver(lons[::stride], lats[::stride], u[::stride,::stride], v[::stride,::stride],pivot='mid',color='Black',scale=400)
...
def main():
    ...
    try:        
        ...
        input_file = 'outfile0211_0.nc'
        output_file0 = 'plots/pres_wind0211_0fig'
        output_file1 = 'plots/rain_wind0122_3fig'
